...
[[https://en.wikipedia.org/wiki/Lucas%27s_theorem][Lucas' Theorem]] states that if \( m_km_{k - 1}m_0 \) and \( n_kn_{k - 1}n_0 \) are the base \( p \) digit representations of \( m \) and \( n \) respectively, then:
